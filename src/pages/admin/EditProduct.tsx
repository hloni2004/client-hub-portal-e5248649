...
export default function EditProduct() {
    ...
    try {
      ...
      if (product.images && product.images.length > 0) {
        const existingImages = product.images.map((img: any) => ({
          imageId: img.imageId,
          preview: `http://localhost:8080/api/products/image/${img.imageId}`,
  ...
  const addColour = () => {
    setColours([...colours, { name: '', hexCode: '#000000', sizes: [{ sizeName: '', stockQuantity: 0 }] }]);
  };
                      ...
                      {colour.sizes.map((size, sizeIndex) => (
                        <div key={sizeIndex} className="flex gap-2">
                          ...
                          <Input
                            ...
                            onChange={(e) => updateSize(colourIndex, sizeIndex, 'stockQuantity', parseInt(e.target.value))}
                          />
                          <Button
                            type="button"
                            variant="ghost"
